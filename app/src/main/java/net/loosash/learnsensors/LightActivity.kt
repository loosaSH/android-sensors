...
class LightActivity : AppCompatActivity(), SensorEventListener {
    ...
    companion object {
        const val TYPE: Int = Sensor.TYPE_LIGHT
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_confirm.setOnClickListener {
            try {
                ...
                Toast.makeText(this@LightActivity, "请输入数值", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        if (event.sensor?.type == TYPE
            ...
            if (Math.abs(xValue - oldX) > referenceValue
                ...
                orientationData.add(0, "光照强度:$xValue lx")
                adapter.notifyDataSetChanged()
            }


//            Log.e("xx", "温度:$xValue °C")
        }
    }
