...
class PressureActivity : AppCompatActivity(), SensorEventListener {
    ...
    companion object {
        const val TYPE:Int = Sensor.TYPE_PRESSURE
    }

    private lateinit var sensorManager: SensorManager
    private var pressureData: ArrayList<String> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_confirm.setOnClickListener {
            try {
                ...
                Toast.makeText(this@PressureActivity, "请输入数值", Toast.LENGTH_SHORT).show()
            }
        }


        adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, android.R.id.text1, pressureData)
    ...
    override fun onSensorChanged(event: SensorEvent) {
        if (event.sensor?.type == TYPE
            ...
            if (Math.abs(xValue - oldX) > referenceValue
                ...
                pressureData.add(0,"大气压强:$xValue hPa")
                adapter.notifyDataSetChanged()
            }


            Log.e("xx", "大气压强:$xValue hPa")
        }
    }
